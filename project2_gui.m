...
function project2_gui()
    ...
    ax1 = uic.Plot(window,[0.07 0.37 0.40 0.58]);
 
    % Add axes on right side for frequency domain plot
    ax2 = uic.Plot(window,[0.55 0.37 0.40 0.58]);
 
    % Add "Frequency" slider control to window
    f_slider = uic.Slider(window,[0.2 0.22 0.6 0.08],[0 100],10,@updateGraph);
 
    % Add "Amplitude" slider control to window
    A_slider = uic.Slider(window,[0.2 0.12 0.6 0.08],[0 10],5,@updateGraph);
 
    % Add "Frequency" edit control to window
    f_edit = uic.Edit(window,18,[0.82 0.22 0.16 0.08]);
 
    % Add "Amplitude" edit control to window
    A_edit = uic.Edit(window,18,[0.82 0.12 0.16 0.08]);
 
    % Add "Frequency" label control to window
    f_label = uic.Label(window,'Frequency:',18,[0.02 0.22 0.16 0.08]);
 
    % Add "Amplitude" label control to window
    A_label = uic.Label(window,'Amplitude:',18,[0.02 0.12 0.16 0.08]);
 
    % Add "Sin" button to window
    sin_button = uic.Button(window,'Sine',18,[0.22 0.02 0.16 0.08],@sinCallback);
 
    % Add "Square" button to window
    sqr_button = uic.Button(window,'Square',18,[0.42 0.02 0.16 0.08],@sqrCallback);
 
    % Add "Saw" button to window
    saw_button = uic.Button(window,'Saw',18,[0.62 0.02 0.16 0.08],@sawCallback);
